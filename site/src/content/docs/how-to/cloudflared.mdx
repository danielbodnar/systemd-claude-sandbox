...
<Aside type="caution">
  The transport choice is a recorded decision that is proposed but not yet accepted; see [transports and the open decision](../../explanation/transports/). That is why cloudflared is double-gated: a compose profile guards the service, and the in-process adapter additionally requires a flag. Treat this guide as opting into the proposal.
</Aside>
