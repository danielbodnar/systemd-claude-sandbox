...
<Aside type="tip">
  On Windows, run the script from Git Bash or WSL. It prefers an installed WSL distro, falls back to VS Code with the devcontainer when Docker is present, and offers an interactive menu when neither exists. Force a mode any time with `CLAUDE_SANDBOX_MODE=compose|devcontainer|bare|wsl`.
</Aside>
