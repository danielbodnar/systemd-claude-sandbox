...
<Aside type="note">
  MCP servers that only the agent needs are best wrapped by the worker as custom tools, which
  requires no inbound connectivity anywhere. The tunnel exists for the other case: humans pointing
  Claude Desktop or claude.ai at servers inside the sandbox network.
</Aside>
