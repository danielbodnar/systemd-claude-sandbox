...
<Aside type="caution">
  Never set `ANTHROPIC_API_KEY` on the worker host. Agent tool calls can read the worker's environment, and the API key is organization-scoped. The upstream integration guide carries the same warning.
</Aside>
